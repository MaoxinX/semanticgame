    ...
    for trialID in ids:
        # try:
            ...
            for doc in docs:
                ...
                block = fields.get('blockN')

                trial = (exp_ID, name, trialnum, currDate_arr, time, category, score, scoretime, switchtime, partshape, partshapenum, trueshape, trueshapenum, result, totalscore, minusscore, money, block)
                # print(trial)
                trials.append(trial)
...
def getTrialData(collection, numTrials, csvFileName, subjects, db):
    ...
    file = open(csvFileName, 'w')
    writer = csv.writer(file)
    header = ('Experiment Name', 'Subject ID', 'trials','Start Time', 'time', 'categoryname', 'score', 'scoretime', 'switchtime', 'partshape', 'partshapenum', 'trueshape', 'trueshapenum', 'result', 'totalscore', 'minusscore', 'money', 'block' )
...
# getSubjectData(subjects,'information/yc009.csv', db, 'Subjects')
getTrialData('Trials',26, 'information/yc103_25.csv', subjects, db)

# Download audio files from the folder
download_audio_files_in_folder('audio-recordings/yc103_25')
download_audio_files_in_folder('transcripts/yc103_25')
